...
ACCESS_KEY = ''
SECRET_KEY = ''
output_file(output_html_name) #bokeh configuration
...
def make_plot(df):
    ...
    'price': df['daiars_price'].tolist(),
    'date' : formatted_date,
    'price_selling':df['daiars_selling'].tolist(),
    'price_purchase':df['daiars_purchase'].tolist()
                            })
    #Drawing the lines
    fig.line(x='date', y='price', line_width=3, color='red', source=source, legend_label='DAI Price')
    fig.line(x='date', y='price_selling', line_width=3, color='blue', source=source, legend_label='DAI selling price')
    fig.line(x='date', y='price_purchase', line_width=3, color='green', source=source, legend_label='DAI purchase price')
    #Configuring hovers
    hover = HoverTool()
...
if __name__ == '__main__':
    ...
    make_plot(df)
    upload_to_aws(output_html_name, bucket, output_html_name)